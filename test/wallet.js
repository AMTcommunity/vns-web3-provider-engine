...
test('tx sig', function(t){
  ...
  var providerC = injectMetrics(new FixtureProvider({
    vns_gasPrice: '0x1234',
    vns_getTransactionCount: '0x00',
    vns_sendRawTransaction: function(payload, next, done){
  ...
  engine.sendAsync(createPayload(txPayload), function(err, response){
    ...
    // intial tx request
    t.equal(providerA.getWitnessed('vns_sendTransaction').length, 1, 'providerA did see "signTransaction"')
    t.equal(providerA.getHandled('vns_sendTransaction').length, 1, 'providerA did handle "signTransaction"')

    // tx nonce
    t.equal(providerB.getWitnessed('vns_getTransactionCount').length, 1, 'providerB did see "vns_getTransactionCount"')
    t.equal(providerB.getHandled('vns_getTransactionCount').length, 0, 'providerB did NOT handle "vns_getTransactionCount"')
    t.equal(providerC.getWitnessed('vns_getTransactionCount').length, 1, 'providerC did see "vns_getTransactionCount"')
    t.equal(providerC.getHandled('vns_getTransactionCount').length, 1, 'providerC did handle "vns_getTransactionCount"')

    // gas price
    t.equal(providerC.getWitnessed('vns_gasPrice').length, 1, 'providerB did see "vns_gasPrice"')
    t.equal(providerC.getHandled('vns_gasPrice').length, 1, 'providerB did handle "vns_gasPrice"')

    // send raw tx
    t.equal(providerC.getWitnessed('vns_sendRawTransaction').length, 1, 'providerC did see "vns_sendRawTransaction"')
    t.equal(providerC.getHandled('vns_sendRawTransaction').length, 1, 'providerC did handle "vns_sendRawTransaction"')
