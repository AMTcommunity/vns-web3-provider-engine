...
//
//   engine.start()
//   engine.sendAsync(createPayload({ method: 'vns_compileSolidity', params: [ contractSource ] }), function(err, response){
...
//
//     t.equal(providerA.getWitnessed('vns_compileSolidity').length, 1, 'providerA did see "vns_compileSolidity"')
//     t.equal(providerA.getHandled('vns_compileSolidity').length, 1, 'providerA did handle "vns_compileSolidity"')
//
//     t.equal(providerB.getWitnessed('vns_compileSolidity').length, 0, 'providerB did NOT see "vns_compileSolidity"')
//     t.equal(providerB.getHandled('vns_compileSolidity').length, 0, 'providerB did NOT handle "vns_compileSolidity"')
//
//     engine.sendAsync(createPayload({ method: 'vns_getCompilers', params: [] }), function(err, response){
//       t.ifError(err, 'did not error')
//       t.ok(response, 'has response')
//
