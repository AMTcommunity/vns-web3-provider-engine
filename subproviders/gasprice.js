...
GaspriceProvider.prototype.handleRequest = function(payload, next, end){
  if (payload.method !== 'vns_gasPrice')
    return next()

  const self = this

  self.emitPayload({ method: 'vns_blockNumber' }, function(err, res) {
    ...
    function getBlock(item, end) {
      self.emitPayload({ method: 'vns_getBlockByNumber', params: [ item, true ] }, function(err, res) {
