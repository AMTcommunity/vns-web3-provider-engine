 ...
 * - vns_getBalance
 * - vns_listTransactions (non-standard)
 */
...
function handlePayload(proto, network, payload, next, end){
  switch(payload.method) {
    case 'vns_blockNumber':
      etherscanXHR(true, proto, network, 'proxy', 'vns_blockNumber', {}, end)
      return

    case 'vns_getBlockByNumber':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getBlockByNumber', {
        tag: payload.params[0],
        boolean: payload.params[1] }, end)
      return

    case 'vns_getBlockTransactionCountByNumber':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getBlockTransactionCountByNumber', {
        tag: payload.params[0]
      }, end)
      return

    case 'vns_getTransactionByHash':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getTransactionByHash', {
    ...
    case 'vns_call':
      etherscanXHR(true, proto, network, 'proxy', 'vns_call', payload.params[0], end)
      return

    case 'vns_sendRawTransaction':
      etherscanXHR(false, proto, network, 'proxy', 'vns_sendRawTransaction', { hex: payload.params[0] }, end)
      return

    case 'vns_getTransactionReceipt':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getTransactionReceipt', { txhash: payload.params[0] }, end)
      return
        ...
        etherscanXHR(true, proto, network, 'proxy', 'vns_getBlockByNumber', {
          ...
          boolean: payload.params[1] }, function(err, blockResult) {
            ...
            for(var transaction in blockResult.transactions){
              etherscanXHR(true, proto, network, 'proxy', 'vns_getTransactionReceipt', { txhash: transaction.hash }, function(err, receiptResult) {
    ...
    case 'vns_getTransactionCount':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getTransactionCount', {
        address: payload.params[0],
    ...
    case 'vns_getCode':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getCode', {
        address: payload.params[0],
        tag: payload.params[1]
      }, end)
      return
    ...
    case 'vns_getStorageAt':
      etherscanXHR(true, proto, network, 'proxy', 'vns_getStorageAt', {
        address: payload.params[0],
