/*
 * Emulate 'vns_accounts' / 'vns_sendTransaction' using 'vns_sendRawTransaction'
 *
...
// Tx Signature Flow
//
// handleRequest: vns_sendTransaction
  ...
  switch(payload.method) {
    ...
    case 'personal_sign':
      return (function(){
        ...
        if (resemblesData(second) && resemblesAddress(first)) {
          let warning = `The vns_personalSign method requires params ordered `
    ...
    case 'vns_decryptMessage':
      return (function(){
        ...
        // and the second param is definitely not, but is hex.
        if (resemblesData(second) && resemblesAddress(first)) {
          let warning = `The vns_decryptMessage method requires params ordered `
          warning += `[message, address]. This was previously handled incorrectly, `
          warning += `and has been corrected automatically. `
          warning += `Please switch this param order for smooth behavior in the future.`
          console.warn(warning)
...
HookedWalletSubprovider.prototype.getGasPrice = function(cb) {
  const self = this
  self.emitPayload({ method: 'vns_gasPrice', params: [] }, function (err, res) {
    if (err) return cb(err)
    cb(null, res.result)
  })
}
...
HookedWalletSubprovider.prototype.fillInTxExtras = function(txParams, cb){
  ...
  if (txParams.nonce === undefined) {
    ...
    tasks.nonce = self.emitPayload.bind(self, { method: 'vns_getTransactionCount', params: [address, 'pending'] })
  }
